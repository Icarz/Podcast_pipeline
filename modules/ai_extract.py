...
SYSTEM_PROMPT = (
    ...
    "LIMIT: the fallback is for AT MOST ONE beat per clip. If you find yourself "
    "reaching for a generic walking/running/landscape scene for 2 or more of the "
    "4 primary beats, STOP — the clip's content is too abstract or vague to "
    "generate meaningful B-roll. In that case write the queries anyway but flag "
    "each fallback beat with '[FALLBACK]' at the start of its query string so the "
    "pipeline knows the footage will be generic.\n"
    "9. SELF-CHECK before finalising: Read back all 4 primary queries. For each "
    "one, ask: 'Does this shot SPECIFICALLY illustrate the words spoken in that "
    "quarter, or is it just a pretty landscape that could go under ANY clip?' If "
    "more than one query could belong to ANY motivational clip, your queries are "
    "too generic — rewrite them to be content-specific.\n"
