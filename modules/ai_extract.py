...
COPY_SYSTEM_PROMPT = (
    ...
    "  - Palette: muted, faded, retro — mustard yellow, olive/forest green, "
    "burnt rust-orange, cream/beige, worn brown leather. Never bright, neon, "
    "or heavily saturated.\n"
    "  - Mood/lighting: dramatic, moody, CINEMATIC lighting is now the "
    "default — silhouettes against a single light source, storm light, "
    "dawn/dusk contrast, deep shadow. The wolf's ACTION stays calm and "
    "controlled; only the LIGHT is allowed to be bold. Flat, evenly-lit "
    "'deadpan' scenes are no longer the default.\n"
    ...
    "second wolf, ever.\n"
    "SCENE: derive the scene from a per-quarter content analysis — for each "
    "of the 4 quarters, name the specific problem/mechanism/insight/payoff "
    "spoken in that quarter (same idea as VIDEO_QUERIES STEP 1), then place "
    "the wolf in whichever of these categories the concept actually maps "
    "to. Rotate across categories across the 4 quarters — never default to "
    "the same one or two every time:\n"
    "  - THRESHOLD/TRANSITION: a doorway, hallway, top/bottom of a "
    "staircase, an open window or ledge — the wolf caught between two "
    "states.\n"
    "  - SOLITUDE-IN-PUBLIC: an empty train platform, an empty diner booth, "
    "a bus stop, a laundromat at night — alone inside a public space built "
    "for crowds.\n"
    "  - WORK/MONEY-AS-FREEDOM: a cluttered desk lit by one lamp, a "
    "workshop bench, counting cash at a kitchen table, packing a bag by the "
    "door — money or work as a means to an exit, never office drudgery.\n"
    "  - NATURE-ADJACENT EXTERIOR: a fire escape, a rooftop ledge, a porch "
    "or balcony overlooking trees or water, a path at the edge of town — "
    "nature visible but the wolf still reads as a person with a home, not "
    "a wilderness scene.\n"
    "  - NIGHT/REST/RUMINATION: sitting bolt upright in bed, staring at a "
    "ceiling fan, pacing a dark room, standing at a window at 3am.\n"
    "  - SMALL RITUALS/AGENCY: lacing boots by the door, folding a letter, "
    "turning a key in a lock, tying a tie in a mirror — a small deliberate "
    "action that signals choice.\n"
    "CINEMATIC COMPOSITION (apply to every prompt): specify a dramatic "
    "camera angle (low angle looking up, high angle looking down, or "
    "framing through a doorway/window), separate foreground and background "
    "so the shot has real depth, and commit to a SINGLE dominant light "
    "source (one lamp, one window, one streetlight, moonlight) rather than "
    "flat even lighting. Add one atmospheric element where it fits the "
    "scene: rain on glass, fog, dust drifting in a light beam, steam, or "
    "falling snow.\n"
    "  Example (threshold/transition): concept 'you keep choosing the "
    "easier path' -> low-angle shot of the wolf standing in an open "
    "doorway at night, backlit by a single porch light, rain falling "
    "through the beam behind him, a dark hallway ahead\n"
    "  Example (solitude-in-public): concept 'everyone around you is "
    "asleep at the wheel' -> wolf alone on a dim subway platform at 2am, "
    "one flickering overhead light, deep shadow swallowing the rest of the "
    "platform\n"
    "  Example (work/money-as-freedom): concept 'money buys you an exit, "
    "not a lifestyle' -> wolf at a cluttered kitchen table late at night, "
    "counting cash under a single hanging bulb, a packed duffel bag "
    "soft-focus by the door\n"
    "  Example (night/rest/rumination): concept 'brain creates fake "
    "scenarios' -> wolf sitting bolt upright in bed, one bedside lamp lit, "
    "a long hard shadow across the wall, staring at nothing\n"
    ...
    "slumped or defeated posture (the wolf's ACTION stays calm/upright even "
    "when the lighting is dramatic); no legible text, signage, or "
    "typography anywhere in the image (captions are composited separately "
    "at render time — baked-in text will clash); no violence or gore; no "
    "crowds or extra figures of any kind.\n"
    "Each of the 4 prompts must be one self-contained, vivid sentence "
    "describing the scene, the wolf's action/posture, the camera angle, and "
    "the lighting — written as a full image-generation prompt, not a short "
    "search query.\n\n"
