...
- **NEVER DEPICT (image scenes) blacklist** — inside the `IMAGE_SCENES` block: no skull/skeleton/death imagery; no cigarettes/alcohol/drugs/vices; no slumped or defeated posture; no violence/gore; no crowds or extra figures of any kind (a busy street/market as an anonymous backdrop is fine — the wolf is the only clearly-rendered figure); no floating/decorative typography. **Contextual PROP TEXT is allowed and encouraged since 2026-07-31 (user decision):** the `action` field may specify 2-6 short readable words for a prop that naturally carries them (a to-do list reading 'GYM - EMAILS - BILLS'), taken from that quarter's idea — verified rendering correctly spelled in the halftone style. Backed in code by `_scene_safety_gate` (regex on `action`/`setting`) and `image_gen.NEGATIVE_BLOCK` in every composed prompt (only the scene-specified wording, on the prop, never anywhere else).
...
- Design system: yellow accent eyebrow with a tick bar, near-white auto-fitting body (`_fit_body` shrinks + re-wraps), persistent footer (wordmark + `TOTAL_SLIDES` (6) progress dots). The giant ghosted serif insight numbers were **removed 2026-07-31** (user request — they competed with the wolf art; insight slides now center their block like the cover, and serif fonts left with them). Fonts: bundled DejaVu Sans Bold in `assets/fonts/` with Windows fallbacks.
