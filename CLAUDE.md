...
.\venv\Scripts\python.exe -m modules.image_gen       # generate gpt-image-2/gradient backgrounds from cached plan (image_prompts) — primary background source
...
1. **OpenAI `gpt-image-2` AI image** (`image_gen.generate_backgrounds`, primary) — generates one on-brief image per `image_prompts` entry (`config.IMAGE_PROMPT_COUNT` = 4) via the OpenAI Images API (`config.OPENAI_IMAGE_MODEL`/`OPENAI_IMAGE_SIZE`/`OPENAI_IMAGE_QUALITY`, currently `gpt-image-2` / `1024x1536` / `medium`); needs `OPENAI_API_KEY`. Retries on 429/5xx with backoff (`RETRY_BACKOFFS`).

   **`image_prompts` art direction (locked brand style, added 2026-07-26, redesigned 2026-07-29, corrected same day — see the `IMAGE_PROMPTS` block in `ai_extract.COPY_SYSTEM_PROMPT`):** every one of the 4 images is a **vintage halftone comic-book illustration** — flat colors, halftone dot shading, bold black ink linework, paper-grain texture; NOT photorealistic. Palette is **WARM AND VIBRANT** (corrected 2026-07-29, reverting a same-day "muted/moody" mistake) — saturated mustard/gold, terracotta/rust orange, vivid kelly green, cream/tan, warm brown wood; think sunlit poster art, never neon but never faded/dark either. The recurring subject is **one anthropomorphic wolf character** — upright, human posture, ordinary clothes, wolf head — always alone (no humans, no other animals, no second wolf). **Mood/lighting: BRIGHT, warm, sunlit/golden-hour is the default** — night/single-lamp/deep-shadow scenes are the rare exception (at most 1 of 4), never the pattern; the wolf's mood reads calm/determined, never sad or shadow-drenched. **Two mandatory scene requirements:** (1) each of the 4 quarters' scene must contain a LITERAL prop or gesture tied to the exact thing being said that quarter (e.g. an actual to-do list held next to a mirror, two jackets held up side-by-side to represent choosing an identity) — a generic mood shot with no concrete link to that quarter's words is a failure regardless of how good it looks; (2) settings must VARY and lean **out-in-the-world over domestic** — gym, street, riding in/driving a car, rooftop, market, park bench, balcony, garage, etc., with home settings (couch/kitchen/desk/doorway) used at most once per clip, never the majority — the wolf is not "stuck at home." **"Cinematic composition"** means dynamic camera angle, depth, and energy (car-window shots, low/high angles, motion blur) — explicitly NOT darkness or gloom; the bright/warm lighting rule always wins. Hard blacklist specific to `image_prompts`: no skull/skeleton/death imagery, no vices, no slumped/defeated posture, no dark/gloomy/night-dominant scenes, **no legible text baked into the image** (the karaoke captions are composited separately in `video_gen.py`; a prop can BE a list/mirror but must not show readable words — gpt-image-1 doesn't always comply with this, e.g. a "TASK LIST" label slipping through, and that's been accepted as a minor known gap rather than re-prompted further), no gore, no extra figures. This is a *separate* blacklist from the human-figure one below, since there's no human figure in these images at all.
...
- `pexels_bg` caches video as **`tmp/bg_<query-sha1[:12]>_<video-id>.mp4`** (keyed by the search query, with the Pexels id embedded — **not** `bg_<n>` by slot index) and slide photos as `tmp/slide_bg_<query-hash>.jpg`; `image_gen` caches `tmp/<episode-basename>_bg_<n>.png` (episode-namespaced — **fixed 2026-07-29**, see gotcha below). Pass `force=True` to regenerate. Because the filename is query-/episode-keyed, a new episode fetches/generates fresh media automatically, while re-rendering the same episode reuses the cached files.
...
- **AI:** `ANTHROPIC_API_KEY` (Claude), `GROQ_API_KEY` (Whisper), `OPENAI_API_KEY` (gpt-image-2 backgrounds — primary background source).
