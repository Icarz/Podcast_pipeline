...
(Deleted visual tracks, both 2026-07-31: the `video_queries` stock-video track — palette tiers, TIER 1/2/3 scene lists, spare-beat system — and the `search_queries` stock-photo track — per-slide Pexels queries, palette rule, NEVER-DEPICT human-figure blacklist, MALE-ONLY rule. Pexels video had already been out of the live background chain since 2026-07-26; Pexels photos left the live slide chain when slides switched to the wolf images. `pexels_bg.py`/`slide_gen`'s legacy Pexels path remain only for old cached plans and standalone harnesses.)
...
- **Backgrounds:** `PEXELS_API_KEY` / `PIXABAY_API_KEY` — **unused by the live pipeline since 2026-07-31** (video backgrounds are gpt-image-2 since 07-26; slides reuse those same images since 07-31). Kept only for `slide_gen`'s legacy Pexels path (old cached plans) and the `pexels_bg`/`pixabay_bg` standalone harnesses.
