...
- **`video_queries`** (exactly 5 `{keyword, query}` objects = **4 primary + 1 spare**) — stock **VIDEO** beats for the clip's moving background. **Keyword-first art direction:** the model first names one core concept keyword per beat (e.g. `focus`, `solitude`, `freedom`) as the emotional anchor, then builds a 2-4 word portrait video-search `query` around it. Tuned for *motion* footage that exists in portrait (walking figures, water/fog/wind, city movement, slow aerials, hands doing things). The first 4 map 1:1 to the 4 background slots; the 5th is a **spare backup** (same tone, distinct scene) that `pexels_bg` only pulls when a primary query yields a duplicate or empty result, so all 4 slots still get distinct footage. All 5 keywords distinct and all 5 queries distinct, tonally consistent so they crossfade as one piece. Consumed by `background.py`, which passes each `.query` to the Pexels video search.

Both follow the same concept→filmable-scene art-director rules in `ai_extract.SYSTEM_PROMPT` (identify the emotion, translate to a real scene a stock shooter actually captured, match the mood not the literal words). The slide rules favor still compositions; the video rules favor motion. The `video_queries` block carries extra numbered rules tuned to keep the crossfading set on-brand: (3) bias every query toward **warm/soft/natural light, calm contemplative energy** — add a treatment word to the query itself (`…warm lamplight`, `…soft morning light`) so the search favors color-graded footage over flat tutorial/corporate B-roll; (5) always depict the **aspirational** version, never the failure-state (negative words like ignorance/fear/giving-up → film the positive counterpart, not defeated/blank/sad subjects); (6) **avoid legible on-screen text** (whiteboards, captioned screens, signage) since it competes with burned-in captions; (7) keep subjects **calm and inward, never performative** (no grinning-at-camera, theatrical gesturing, mid-laugh); (8) **tonal-palette lock** — all 4 primary beats + the spare must share ONE palette (`warm-natural` / `cool-cinematic` / `warm-interior`); mixing palettes across slots is forbidden, so the four clips read as one film, not four accounts; (9) the `keyword` must be **aspirational/neutral, never negative/passive** (no `overwhelm`/`fear`/`anxiety`/`defeat`/`exhaustion` — a negative keyword pulls defeated footage even when the query reads fine; use the counterpart e.g. `clarity`, `momentum`, `stillness`); (10) **safe-scene fallback** for abstract concepts with no filmable scene (lone runner on a forest path, hands writing in a notebook, figure in morning mist, sunrise from a high vantage, slow aerial over mountains/coast) — don't literalize, pick the safe scene closest in mood. (11) is the `Format:` line. Keep rule numbering contiguous and the `Format:` line last when editing.

**Cross-cutting prompt guards (added after production renders surfaced off-brand footage):**
- **NEVER DEPICT hard blacklist** — a shared block (just before the `SEARCH_QUERIES` section) that binds **both** `video_queries` AND `search_queries`: no hunched/slumped/head-down/seated-in-defeat postures (body must read upright, in motion, or in calm purposeful stillness); no smoking/vaping/alcohol/drugs/junk food or any vice; no readable signage/graffiti/legible book text/flipcharts/on-screen words; no crowds/stadiums/conferences/audiences; no identifiable-person face close-ups.
...
- **`insights` ≤ 110 chars** — hard cap in the `insights` schema line so `slide_gen._fit_body` never has to shrink copy to thumbnail-illegible size.
