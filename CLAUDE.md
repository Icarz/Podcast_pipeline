...
1. `script_gen.generate_script_with_retry()` — one Claude call: picks a topic
   (informed by `script_history.recent(8)`, favoring the 3 proven clusters —
...
- `script_gen.generate_script_with_retry(attempts=3)` → wraps `generate_script(recent_history)`, catching only `ValueError` (schema/brand-gate failure) and `anthropic.RateLimitError`, same 3-attempt/65s-sleep retry shape the old `extract_copy_with_retry` used. Returns exactly these keys:
  ...
  - `topic_cluster` is one of `config.TOPIC_CLUSTERS` (5 fixed buckets: `fear_anxiety_rumination`, `individuality_vs_conformity`, `money_as_freedom` — the 3 proven performers, favored in the prompt — plus `neurology_focus_motivation`, `identity_resilience_meaning`). Used only for the dedup ledger; the model still free-picks a specific topic each run.
...
**Proven topic clusters, favored by `script_gen`'s prompt:** fear/anxiety/rumination mechanisms are the single strongest recurring sub-topic (3 of the last 6 top performers, back when this was sourced from podcasts); individuality-vs-conformity ("the courage to want more than the crowd finds acceptable") is a second proven cluster; money reframed as freedom is a validated third cluster. These three map directly onto `config.TOPIC_CLUSTERS`' first three entries.
...
- **Hook formula (contrarian identity frame)** — the `hook` field must use one of several proven structures that challenge the viewer's worldview and imply they are on the wrong side of a divide. Instructional hooks ("X tips for Y", "How to…", "The science of…") are explicitly banned. The hook must be under 15 words.
...
- **Script length is a soft target, not an enforced window.** `script_gen`'s prompt targets `config.CLIP_WINDOW_MIN_SECONDS`-`CLIP_WINDOW_MAX_HARD_SECONDS` (45-58s) of spoken narration, but there's no transcript to snap to anymore — the REAL, final duration is simply whatever the ElevenLabs voiceover comes out to. `video_gen.build_video` clamps `clip_end` to the actual audio duration regardless of what's cached (`highlights["clip_end"]` is always the sentinel `9999.0`). Proven fine at 39.7s in the first manual test render, well under either bound.
