...
NEGATIVE_BLOCK = (
    ...
    "the image. That prop text must be shown face-on and fully legible from "
    "the camera's viewpoint — never at an angle, foreshortened, upside-down, "
    "mirrored, or turned away from the viewer. No skulls or death imagery, "
    "no cigarettes, alcohol, drugs, or vices, no slumped or defeated "
    "posture, no violence or gore. No humans, no other animals, no second "
    "wolf — the wolf is the only figure (a busy street or market as a soft "
    "anonymous backdrop is fine, with no other clearly rendered figure). The "
    "wolf's design has NO visible tail in any scene — fully human proportions "
    "and silhouette below the neck, wolf head and hands only."
)
