...
- **Extraction is non-deterministic — always wrap re-extracts in a retry loop.** Identical transcript inputs yield varying output: occasional trailing data after the JSON (the model appends a note/second object), or off-by-one counts (e.g. 5 `image_prompts` instead of 4) that trip `_validate`. `_strip_to_json` now isolates the **first complete JSON object** via `json.JSONDecoder().raw_decode` (tolerates trailing data); the count/validation variance is not caught, so loop `extract_highlights` a few times until one passes rather than failing on the first throw.
- **Final-slot background pillarboxing (open bug).** The last video slot can show **black side bars that grow toward the very end** (last ~4s): its Ken Burns/cover-crop scale drifts below full 1080-wide coverage. Slots 1–3 hide the same drift under the crossfade into the next clip; the final slot has no successor so its bare tail is exposed. Fix lives in `video_gen`'s video-slot Ken Burns path — clamp min effective scale so the cover-crop scale is the floor. **Verify every render** by frame-grabbing the tail, not just the middle.
