...
SYSTEM_PROMPT = (
    ...
    "is hijacking you with/is fighting to keep you exactly the same, "
    "defending X against] [common behavior] -- [until/unless] [condition]' "
    ...
    "The 'actively DEFENDS/PROTECTS the status quo against you' shape "
    "(homeostasis / ego-depletion / amygdala-hijack -- the system isn't "
    "just doing its own thing, it's working AGAINST a goal you actually "
    "want) is a confirmed top performer within this default (62.9% AVD on "
    "real reach, 2026-08-12) -- reach for it over a vaguer 'X does Y' "
    "framing when the script's mechanism genuinely resists a viewer goal.\n"
    ...
    "action it expresses, AND identify a short fragment (3-8 words, "
    "near-verbatim) actually taken from that quarter's script text as "
    "proof the concept is really there -- the literal words, not a "
    "paraphrase of the general topic. Do not wrap this fragment in "
    "quotation-mark punctuation anywhere below -- state it as plain text. "
    "(Quotation marks embedded inside a JSON string value are a common "
    "cause of broken JSON output if not escaped correctly; plain text "
    ...
    "STEP 1 this scene illustrates, working in that quarter's anchor "
    "fragment as plain text (no quotation-mark punctuation around it) so "
    "the connection to the actual script is checkable at a glance -- e.g. "
    "something like: the raise-fueled upgrade to a nicer apartment and "
    "car still leaves him feeling no richer.\n"
    ...
    "  FACING RULE (physical consistency): a screen or page only has one "
    "readable side. If the wolf is the one looking at the prop (checking "
    "his phone, reading a note, writing on a pad), the readable side MUST "
    "face the wolf, NOT the camera -- say so explicitly (e.g. 'glances at "
    "his phone, screen angled toward him, back of the phone to camera'). "
    "Never describe the wolf as looking at a prop while also describing "
    "that prop as facing the viewer -- that puts the readable side away "
    "from the one person supposedly reading it, which is physically "
    "impossible. A prop CAN face the camera only when nobody in the scene "
    "is depicted reading it (a shop sign in the background, a receipt left "
    "on a counter, a note already pinned to a wall).\n"
    "  SCREENS: if the prop is a phone, laptop, tablet, smartwatch, or any "
    "other device with a screen, NEVER describe or imply what is on the "
    "screen (no balances, notifications, app UI, or numbers) -- the image "
    "model will invent its own specific text/figures there, and an invented "
    "number can flatly contradict the script (e.g. a '$0.00 balance' right "
    "after buying a car reads as 'actually broke', not the intended "
    "'still doesn't feel richer'). The device itself is the prop, never "
    "its display -- combine this with the FACING RULE above (screen "
    "angled toward the wolf, away from camera).\n"
    "  WRITTEN PROP TEXT: at most ONE of the 6 scenes may put legible "
    "words on a prop, and only per the FACING RULE above -- a prop nobody "
    "is depicted reading (a shop sign, a receipt, a package label, a "
    "whiteboard already on a wall), never an object the wolf holds up, "
    "carries, or angles toward the camera specifically to display it (no "
    "sign-holding, no carrying a notebook at his side to show its cover), "
    "and never the same prop reused across more than one scene. If you use "
