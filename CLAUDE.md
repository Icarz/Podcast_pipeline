...
- `ai_extract.extract_highlights(transcript)` → validated JSON dict with exactly these keys:
  ...
  - `_trim_to_cap()` runs **before** `_validate` (and again after the sentence snap): if the model insists on a complete thought that runs past the hard cap (extraction is deterministic, so re-asking returns the same pick), it pulls `clip_end` back to the latest sentence-ending word under the cap rather than rejecting the clip.
