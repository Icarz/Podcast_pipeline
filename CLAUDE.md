...
**Dedicated YouTube Shorts cover/thumbnail generation was removed on 2026-08-16** (user decision — no longer wanted). `modules/thumbnail_gen.py` was deleted; `script_gen`'s `thumbnail_text` field, `image_gen`'s `compose_thumbnail_prompt`/`THUMBNAIL_*` constants, and `config`'s `THUMBNAIL_*` constants were removed with it. The pipeline's only outputs now are the karaoke MP4 and the 6-slide carousel — no 7th image, no poster-text overlay. See git history if a Shorts-cover step is ever wanted back.
...
**Updated Aug 13, 2026 — best synthetic-era result, causal breakdown:** "Comfort Isn't Rest — It's Your Body Sabotaging Your Goals" (topic cluster `neurology_focus_motivation`, posted 2026-08-12) hit **62.9% native average-percentage-viewed** on real reach (1.3K views, #1 of 10 ranked Shorts) — the best result of the synthetic-script era and inside the all-time-best 55-69% band. Traced to what was actually active for that generation (`plan.json` written 22:54, before the same-day metaphor-payoff-gate commit landed at 23:42 — that gate gets no credit here): the existing DEFAULT NEURO hook formula executed cleanly ("Your nervous system fights your goals to keep you exactly the same"), the 2026-08-02 NAMED MECHANISM RULE (homeostasis named explicitly), an immediate 2nd-person concrete opening with no throat-clearing ("You think you're lazy. You're not."), and the 2026-08-06 situation-first `IMAGE_SCENES` rewrite (`116d94f`) producing real recognizable moments instead of prop-first product shots. **No new prompt rule was needed to produce this win** — it validates rules already in place. One small additive change was made afterward: the "actively DEFENDS/PROTECTS the status quo against you" shape (homeostasis/ego-depletion/amygdala-hijack — the mechanism resists a goal the viewer actually wants) was folded into the WINNING FORMULA as an explicit named variant of the existing DEFAULT tier, not a new competing formula. Treat it as a confirmed candidate, not yet proven independent of the other ingredients in this one render — reassess after 2-3 more data points. Full breakdown and the corrected-attribution note live in `docs/retention_log.md`'s 2026-08-13 entry.
...
- **Retention tracking file** — per published short: hook family, topic cluster, then retention % filled in weekly; average-view-duration % is the metric that predicts Shorts distribution. (File exists at `docs/retention_log.md` as of 2026-08-12 — keep appending rows here, this item is about maintaining the habit, not creating the file.)
